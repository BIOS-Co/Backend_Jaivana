...
def recomendar_productos(request_data: RecomendarProductosRequest):
    ...
    if campos_ingresados[-1]["Ciiu"]!=None:
      campos_ingresados[-1]["Ciiu"]=int(campos_ingresados[-1]["Ciiu"])
...
class Pricing_Request(BaseModel):
    ...
    Descuento_1:str=None
    Descuento_2:str=None
    Descuento_3:str=None
    Descuento_4:str=None
    Descuento_5:str=None
    Descuento_Inicial:str=None
    Nit:str
    Umbral_Iteracciones:str
    grupo:str=None
    subgrupo:str=None
    tipo_calculo:str
...
def fijar_precios(request_data: Pricing_Request):
  ...
  Descuento_1=request_data.Descuento_1
  Descuento_2=request_data.Descuento_2
  Descuento_3=request_data.Descuento_3
  Descuento_4=request_data.Descuento_4
  Descuento_5=request_data.Descuento_5
  Descuento_Inicial=request_data.Descuento_Inicial
  ...
  tipo_calculo=request_data.tipo_calculo
  
  if Descuento_1!=None:
    Descuento_1=float(Descuento_1)/100
  else:
    Descuento_1="Predefinido"
    
  if Descuento_2!=None:
    Descuento_2=float(Descuento_2)/100
  else:
    Descuento_2="Predefinido"
    
  if Descuento_3!=None:
    Descuento_3=float(Descuento_3)/100
  else:
    Descuento_3="Predefinido"
    
  if Descuento_4!=None:
    Descuento_4=float(Descuento_4)/100
  else:
    Descuento_4="Predefinido"
    
  if Descuento_5!=None:
    Descuento_5=float(Descuento_5)/100
  else:
    Descuento_5="Predefinido"
    
  if Descuento_Inicial!=None:
    Descuento_Inicial=float(Descuento_Inicial)/100
  else:
    Descuento_Inicial="Predefinido"
    
  Umbral_Iteracciones=int(Umbral_Iteracciones)
  
  if tipo_calculo=="Calcular a partir de ultimas importaciones":
    tabla_principal="sumatec_itemfc_compras"
  else:
    tabla_principal="sumatec_items_pedidos_compras"
  ...
  if opcion_seleccionada=='Precio de un Producto':      
      ...
      try:
          ...
          if opcion_seleccionada_tipo_iteraccion=="Iterar por Historial Proveedor":
              query = f"SELECT codigo, cantidad, costo, valor_iva, valor_item, fecha, nit FROM {tabla_principal} WHERE nit='{Nit}' AND fecha IN (SELECT DISTINCT fecha FROM {tabla_principal} WHERE nit='{Nit}' ORDER BY fecha DESC LIMIT 5) ORDER BY fecha DESC;"
              ...
              if itemfc_compras.shape[0]==0:
                  ...
                  for i in range(Umbral_Iteracciones):
                      ...
                      if Codigo_Tornillo in list(itemfc_compras_temporal["codigo"].unique()):
                          ...
                          if i==0:
                              query_2 = f"SELECT codigo, nombre, codigo_grupo, nombre_grupo, codigo_subgrupo, nombre_subgrupo, peso FROM sumatec_articulo where codigo='{Codigo_Tornillo}'"
                              bi_articulo = pd.read_sql_query(query_2, connection)
                              if bi_articulo.shape[0]==0:
                                  output_costo="No se puede fijar un precio al producto; ya que no se encuentra información suficiente en base de datos para calibrar la curva de su costo, en este caso debido a que el producto no se encuentra en la tabla sumatec_articulo"
                                  break
                              else:
                                  ...
                                  peso=float(peso)
                                  query_codigos = f"SELECT DISTINCT(codigo) FROM sumatec_articulo where codigo_grupo='{codigo_grupo}' AND codigo_subgrupo='{codigo_subgrupo}'"
                                  ...
                                  if itemfc_compras_temporal_temporal.shape[0]!=0:
                                      ...
                                      query_3 = f"SELECT codigo, nombre, codigo_grupo, nombre_grupo, codigo_subgrupo, nombre_subgrupo, peso FROM sumatec_articulo where codigo='{codigo_producto_alterno}'"
              ...
              provisional_query = f"SELECT codigo, nombre, codigo_grupo, nombre_grupo, codigo_subgrupo, nombre_subgrupo, peso FROM sumatec_articulo where codigo='{Codigo_Tornillo}'"
              ...
              if provisional_bi_articulo.shape==0:
                  ...
                  query = f"SELECT codigo, cantidad, costo, valor_iva, valor_item, fecha, nit FROM {tabla_principal} WHERE codigo IN ({codigos_str_provisional}) AND fecha IN (SELECT DISTINCT fecha FROM {tabla_principal} WHERE codigo IN ({codigos_str_provisional}) ORDER BY fecha DESC LIMIT 5) ORDER BY fecha DESC;"
                  itemfc_compras = pd.read_sql_query(query, connection)    
          ...
          if isinstance(output_costo, str):
              ...
              if Descuento_Inicial=="Predefinido":
                  ...
                  precio_lista_5=output_costo/(Descuento_Inicial*0.65)
                  precio_lista_4=precio_lista_5
                  precio_lista_1=precio_lista_5/0.8
                  precio_lista_2=precio_lista_1
                  precio_lista_3=0.9*precio_lista_1
                  
              if Descuento_1=="Predefinido":
                precio_lista_1=precio_lista_1-precio_lista_1*descuentos_listas["lista1"]
              else:
                precio_lista_1=precio_lista_1-precio_lista_1*Descuento_1
                
              if Descuento_2=="Predefinido":
                precio_lista_2=precio_lista_2-precio_lista_2*descuentos_listas["lista2"]
              else:
                precio_lista_2=precio_lista_2-precio_lista_2*Descuento_2
                
              if Descuento_3=="Predefinido":
                precio_lista_3=precio_lista_3-precio_lista_3*descuentos_listas["lista3"]
              else:
                precio_lista_3=precio_lista_3-precio_lista_3*Descuento_3
                
              if Descuento_4=="Predefinido":
                precio_lista_4=precio_lista_4-precio_lista_4*descuentos_listas["lista4"]
              else:
                precio_lista_4=precio_lista_4-precio_lista_4*Descuento_4
                
              if Descuento_5=="Predefinido":
                precio_lista_5=precio_lista_5-precio_lista_5*descuentos_listas["lista5"]
              else:
                precio_lista_5=precio_lista_5-precio_lista_5*Descuento_5
              ...
              # Redondear solo las columnas especificadas a 2 decimales
              output_precio[["Precio_Venta"]] = output_precio[["Precio_Venta"]].round(2)
      ...
      try:
          ...
          output_costo=None
          # Ejecutar una consulta y cargar los resultados en un DataFrame
          query = f"SELECT codigo, nombre, codigo_grupo, nombre_grupo, codigo_subgrupo, nombre_subgrupo, peso FROM sumatec_articulo where codigo_grupo='{grupo}' AND codigo_subgrupo='{subgrupo}'"
          bi_articulo = pd.read_sql_query(query, connection)
          if bi_articulo.shape[0]==0:
              output_costo="No es posible calcular precios; ya que en la tabla sumatec_articulo no existe ningún articulo perteneciente al grupo y subgrupo indicado"
              ultima_fecha_iterada="No fue necesario iterar a través de ninguna fecha"
          else:
              if opcion_seleccionada_tipo_iteraccion=="Iterar por Historial Proveedor":
          ...
          if isinstance(output_costo, pd.DataFrame):
              ...
              if Descuento_Inicial=="Predefinido":
                  ...
                  for index in output_costo.index:
                      output_costo["Precio_Venta_Lista_5"].loc[index]=output_costo["Costo_Unitario"].loc[index]/(Descuento_Inicial*0.65)
                      output_costo["Precio_Venta_Lista_4"].loc[index]=output_costo["Precio_Venta_Lista_5"].loc[index]
                      output_costo["Precio_Venta_Lista_1"].loc[index]=output_costo["Precio_Venta_Lista_5"].loc[index]/0.8
                      output_costo["Precio_Venta_Lista_2"].loc[index]=output_costo["Precio_Venta_Lista_1"].loc[index]
                      output_costo["Precio_Venta_Lista_3"].loc[index]=output_costo["Precio_Venta_Lista_1"].loc[index]*0.9
              ...
              if Descuento_1=="Predefinido":
                ...
                output_costo["Precio_Venta_Lista_1"]=output_costo["Precio_Venta_Lista_1"]-output_costo["Precio_Venta_Lista_1"]*Descuento_1
              ...
              if Descuento_2=="Predefinido":
                ...
                output_costo["Precio_Venta_Lista_2"]=output_costo["Precio_Venta_Lista_2"]-output_costo["Precio_Venta_Lista_2"]*Descuento_2
              ...
              if Descuento_3=="Predefinido":
                ...
                output_costo["Precio_Venta_Lista_3"]=output_costo["Precio_Venta_Lista_3"]-output_costo["Precio_Venta_Lista_3"]*Descuento_3
              ...
              if Descuento_4=="Predefinido":
                ...
                output_costo["Precio_Venta_Lista_4"]=output_costo["Precio_Venta_Lista_4"]-output_costo["Precio_Venta_Lista_4"]*Descuento_4
              ...
              if Descuento_5=="Predefinido":
                ...
                output_costo["Precio_Venta_Lista_5"]=output_costo["Precio_Venta_Lista_5"]-output_costo["Precio_Venta_Lista_5"]*Descuento_5
                
              
              output_precio=output_costo
              condicion_codigos_no_validos = ~output_precio['codigo'].str.startswith(('0000', '9999'))
              output_precio=output_precio[condicion_codigos_no_validos]
              
              condicion_nombre_arroa = ~output_precio['nombre'].str.contains('@')
              output_precio=output_precio[condicion_nombre_arroa]
              
              columnas_a_redondear = ['Costo_Unitario', 'Precio_Venta_Lista_1', 'Precio_Venta_Lista_2','Precio_Venta_Lista_3'
                                      ,'Precio_Venta_Lista_4','Precio_Venta_Lista_5']

              # Redondear solo las columnas especificadas a 2 decimales
              output_precio[columnas_a_redondear] = output_precio[columnas_a_redondear].round(2)
  ...
  if isinstance(output_precio, pd.core.frame.DataFrame):
    csv=output_precio.to_csv(index=False)
    return {"ultima_fecha_iterada":ultima_fecha_iterada,"output_precio":output_precio.to_dict(orient="records"),"csv":csv}  
  else:
    return {"ultima_fecha_iterada":ultima_fecha_iterada,"output_precio":output_precio,"csv":""}      
